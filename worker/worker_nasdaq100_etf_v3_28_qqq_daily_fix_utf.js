...
// ===== v3.27 OTC cache upgrade helpers =====
function getOtcCacheDateV27(){
...
function getOtcCacheInfoV27(source,generatedAt,refreshMode){
  return {
    source: source || "D1",
    date: getOtcCacheDateV27(),
    generatedAt: generatedAt || null,
    refreshMode: refreshMode || "daily"
  };
}

async function ensureOtcDailyCacheTableV27(env){
  if(!env.DB) return;

  await env.DB.prepare(`
    CREATE TABLE IF NOT EXISTS otc_daily_cache(
...
async function readOtcDailyCacheV27(env){
  if(!env.DB) return null;

  await ensureOtcDailyCacheTableV27(env);

  const date=getOtcCacheDateV27();

  const row=await env.DB.prepare(
    "SELECT data_json,updated_at FROM otc_daily_cache WHERE cache_date=?"
  ).bind(date).first();
...
async function writeOtcDailyCacheV27(env,data,refreshMode){
  if(!env.DB) return;
  ...
  await env.DB.prepare(`
    INSERT OR REPLACE INTO otc_daily_cache
    (
      cache_date,
      data_json,
      created_at,
...
// ===== v3.27 OTC cache upgrade end =====



// ================= v3.28 OTC complete upgrade =================

// D1 cache information
function otcV28ShanghaiDate(){
  return new Intl.DateTimeFormat("en-CA",{
    timeZone:"Asia/Shanghai",
    year:"numeric",
    month:"2-digit",
...
function otcV28CacheInfo(mode,updatedAt){
  return {
    source:"D1",
    cacheDate:otcV28ShanghaiDate(),
    generatedAt:updatedAt || new Date().toISOString(),
    refreshMode:mode || "daily"
  };
}

// Keep refresh mode parser independent from old API.
function otcV28RefreshMode(url){
  const refresh=url.searchParams.get("refresh");
  if(refresh==="quota") return "quota";
  if(refresh==="announcement") return "announcement";
  if(refresh==="1") return "full";
  return "daily";
}

// Cloudflare Cron entry helper.
// The actual fetch function remains compatible with existing OTC engine.
async function otcV28CronRefresh(env){
  try{
    if(typeof buildOtcFundsHybridV37==="function"){
      const result=await buildOtcFundsHybridV37({forceRefresh:true});
      if(env.DB){
        const now=new Date().toISOString();
        await env.DB.prepare(`
          CREATE TABLE IF NOT EXISTS otc_daily_cache(
        ...
        await env.DB.prepare(`
          INSERT OR REPLACE INTO otc_daily_cache
          (cache_date,data_json,created_at,updated_at)
          VALUES(?,?,?,?)
        `).bind(
          otcV28ShanghaiDate(),
      ...
      return {ok:true,refreshMode:"cron"};
    }
    return {ok:false,error:"OTC engine not found"};
  }catch(e){
    return {ok:false,error:e?.message||String(e)};
  }
}

// ================= v3.28 OTC complete upgrade end =================
